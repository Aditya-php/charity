...
if(!function_exists('checkUserExistenceInAllStages'))
{
  function checkUserExistenceInAllStages($user_id=null)
  {
	 ...
	 $exist['exist_in_stage7']=($stage7_level1_info->num_rows()==2 && $stage7_level2_info->num_rows()==4)?1:0;
	 ...
	 $exist['exist_in_stage8']=($stage8_level1_info->num_rows()==2 && $stage8_level2_info->num_rows()==4)?1:0;
...
if(!function_exists('get_user_rank_image'))
{
  function get_user_rank_image($user_id=null)
  {
      ...
      $exist_in_stage5=($stage5_level1_info->num_rows()==2 && $stage5_level2_info->num_rows()==4)?1:0;
      ...
      $exist_in_stage6=($stage6_level1_info->num_rows()==2 && $stage6_level2_info->num_rows()==4)?1:0;
	  ///////stage7 login user existence
	  
	  $stage7_level1_info=$obj->db->select('id')->from('matrix_stage6')->where(array('income_id'=>$user_id, 'level'=>'1'))->get();
      
	  $stage7_level2_info=$obj->db->select('id')->from('matrix_stage6')->where(array('income_id'=>$user_id, 'level'=>'2'))->get();
	  
	  $exist_in_stage7=($stage7_level1_info->num_rows()==2 && $stage7_level2_info->num_rows()==4)?1:0;
	  
	  ///////stage8 login user existence
	  
	  $stage8_level1_info=$obj->db->select('id')->from('matrix_stage7')->where(array('income_id'=>$user_id, 'level'=>'1'))->get();
      
	  $stage8_level2_info=$obj->db->select('id')->from('matrix_stage7')->where(array('income_id'=>$user_id, 'level'=>'2'))->get();
	  
	  $exist_in_stage8=($stage8_level1_info->num_rows()==2 && $stage8_level2_info->num_rows()==4)?1:0;
	  
      $image_name="stage1.png";
	  
	  if($exist_in_stage1)
	  {
	  $image_name="stage2.png";
	  }
	  if($exist_in_stage2)
	  {
		$image_name="stage3.png";
	  }
	  if($exist_in_stage3)
	  {
		$image_name="stage4.png";
	  }
	  if($exist_in_stage4)
	  {
		$image_name="stage5.png";
	  }
	  if($exist_in_stage5)
	  {
		$image_name="stage6.png";
	  }
	  if($exist_in_stage6)
	  {
		$image_name="stage7.png";
	  }
	  if($exist_in_stage7)
	  {
		$image_name="stage8.png";
	  }
      return $image_name;
  }
}//end method
