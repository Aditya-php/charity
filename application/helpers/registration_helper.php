			    ...
			    case '1':
					{
					  ...
					  $left_leg_exist=$obj->db->select('nom_leg_position')->from('user_registration')->where(array('nom_id'=>$nom_id1,'nom_leg_position'=>'left'))->get()->num_rows();
					  
					  $right_leg_exist=$obj->db->select('nom_leg_position')->from('user_registration')->where(array('nom_id'=>$nom_id1,'nom_leg_position'=>'right'))->get()->num_rows();
					  
					  if($left_leg_exist<=0 && $right_leg_exist>0)
					  {
						  $nom_leg_position='left';
					  }
			    ...
			    case '1':
					{
					  ...
					  $left_leg_exist=$obj->db->select('nom_leg_position')->from($table_name)->where(array('nom_id'=>$nom_id1,'nom_leg_position'=>'left'))->get()->num_rows();
					  
					  $right_leg_exist=$obj->db->select('nom_leg_position')->from($table_name)->where(array('nom_id'=>$nom_id1,'nom_leg_position'=>'right'))->get()->num_rows();
...
function matrix_commission_level($total_com,$down_id,$table_name,$stage_name)
{
	...
	$query=$obj->db->select('*')->from($table_name)->where(array('down_id'=>$down_id,'level <='=>$level, 'matrix_commission_level'=>'Unpaid'))->get();
...
if(!function_exists('ewalletUserRegistration'))
{
   function ewalletUserRegistration($registration_info=null)
   {
	...
	$query_obj=$obj->db->select('amount')->from('final_e_wallet')->where('user_id',$sponser_id)->get()->row();
	$available_amount=$query_obj->amount;
	if($pkg_amount>$available_amount)
	{
...
function check_upliners7($new_id,$platform)
{
		...
		foreach($info as $row)
		{
			...
			$lev3_num=$obj->db->select('*')->from('matrix_stage6')->where(array('income_id'=>$upliner,'level'=>'3'))->get()->num_rows();
			//////////////////////////////////////////////
			...
			$lev3_num_info=$obj->db->select('*')->from('matrix_stage6')->where(array('income_id'=>$upliner,'level'=>'3'))->limit('1','0')->get()->row();
			...
			if($lev1_num>=2 && $lev2_num>=4 && $lev3_num>=8)
			{
					if($lev1_num_info->pay_status=='Unpaid' && $lev2_num_info->pay_status=='Unpaid' && $lev3_num_info->pay_status=='Unpaid')
				    {
						...
						if($incentive_exist==0)
						{
						 ...
						 $obj->db->update('matrix_stage6',array('pay_status'=>'Paid'),array('income_id'=>$upliner,'level'=>'3'));
...
function check_upliners8($new_id,$platform)
{
		...
		foreach($info as $row)
		{
			...
			$lev3_num=$obj->db->select('*')->from('matrix_stage7')->where(array('income_id'=>$upliner,'level'=>'3'))->get()->num_rows();
			...
			$lev3_num_info=$obj->db->select('*')->from('matrix_stage7')->where(array('income_id'=>$upliner,'level'=>'3'))->limit('1','0')->get()->row();
					...
					if($lev1_num_info->pay_status=='Unpaid' && $lev2_num_info->pay_status=='Unpaid' && $lev3_num_info->pay_status=='Unpaid')
				    {
						...
						///////////////////////incentive/////////////////////////////////
						 ...
						 $obj->db->update('matrix_stage7',array('pay_status'=>'Paid'),array('income_id'=>$upliner,'level'=>'2'));
						  ...
						  $obj->db->update('matrix_stage7',array('pay_status'=>'Paid'),array('income_id'=>$upliner,'level'=>'3'));
					}
			  //all stage matrix completion code here
			}//end level if
	    }//end foreach
...
function sendNewRegistrationEmailToAdmin($user_id,$username,$password,$sponsor_username,$upliner,$email)
{
    ...
    $email_data['from']='admin@charitybeginsnathi.co.za';
    $email_data['to']='admin@charitybeginsnathi.co.za';
...
{
   function testRegistration($sp_name,$username,$ref_leg_position)
   {
    $obj=& get_instance();
    ...
    $sp_info=$obj->db->select('*')->from('user_registration')->where('username',$sp_name)->get()->row();
	
   
	$sponser_id=$sp_info->user_id;
	$nom_id_info=getFollowMeMatrixNom($sponser_id,$ref_leg_position);
	//pr($nom_id_info);
	echo "nom_id=".$nom_id=$nom_id_info['nom_id'];;
	echo "<br>username=".$obj->db->select('username')->from('user_registration')->where('user_id',$nom_id)->get()->row()->username;
	
	$nom_id1=$nom_id;
	echo "<br>pos=".$nom_leg_position=$nom_id_info['nom_leg_position'];
...
if(!function_exists('testUserRegistration'))
{
   function testUserRegistration($row_array)
   {
    $obj=& get_instance();
	$registration_info=$obj->session->userdata('registration_info');
	$sponser_id=$row_array->ref_id;
	...
	$pkg_id=$row_array->pkg_id;
	$pkg_amount=$row_array->pkg_amount;
	$username=$row_array->username;
	$user_password=$row_array->password;
	$transaction_pwd=$row_array->t_code;
	$user_id=(!empty($row_array->user_id))?$row_array->user_id:null;
	
	//personal informtaion
	
	$first_name=(!empty($row_array->first_name))?$row_array->first_name:null;
	$last_name=(!empty($row_array->last_name))?$row_array->last_name:null;
	$email=(!empty($row_array->email))?$row_array->email:null;
	$contact_no=(!empty($row_array->contact_no))?$row_array->contact_no:null;
	$country=(!empty($row_array->country))?$row_array->country:null;
	
	$state=(!empty($row_array->state))?$row_array->state:null;
	
	
	$city=(!empty($row_array->city))?$row_array->city:null;
	$zip_code=(!empty($registration_info['personal_info']['zip_code']))?$registration_info['personal_info']['zip_code']:null;
	$address_line1=(!empty($registration_info['personal_info']['address_line1']))?$registration_info['personal_info']['address_line1']:null;
	$date_of_birth=(!empty($registration_info['personal_info']['date_of_birth']))?$registration_info['personal_info']['date_of_birth']:null;
    ...
    $user_registration_data=array(
    		...
    		'pkg_amount'=>$pkg_amount,
    		 ...
    		 'address_line1'=>$date_of_birth,
    		 /*Bank Account information*/
    		 'account_no'=>(!empty($row_array->account_no))?$row_array->account_no:null,
    		 'branch_name'=>(!empty($row_array->branch_name))?$row_array->branch_name:null,
    		 'bank_name'=>(!empty($row_array->bank_name))?$row_array->bank_name:null,
    		 'ifsc_code'=>(!empty($row_array->ifsc_code))?$row_array->ifsc_code:null,
    		 'account_holder_name'=>(!empty($row_array->account_holder_name))?$row_array->account_holder_name:null,
    		 ////////
    		 'registration_date'=>(!empty($row_array->registration_date))?$row_array->registration_date:null,
    		 'current_login_status'=>'0',
    		 'current_login_status'=>'0', 
    		 'active_status'=>'1',
			 'registration_method'=>(!empty($row_array->registration_method))?$row_array->registration_method:null,
			 'registration_method_name'=>(!empty($row_array->registration_method_name))?$row_array->registration_method_name:null
    		);
	...
	if(!empty($row_array->registration_method_name) && $row_array->registration_method_name=='E-wallet')
	{
		$query_obj=$obj->db->select('amount')->from('final_e_wallet')->where('user_id',$sponser_id)->get()->row();
		$balance=$query_obj->amount-$pkg_amount;
		$obj->db->update('final_e_wallet',array('amount'=>$balance),array('user_id'=>$sponser_id));
...
function check_upliners1_manual($info,$platform)
{
		$obj=& get_instance();
		$commission_info=$obj->db->select('*')->from('matrix_stage_commission_meta')->where(array('pkg_id'=>$platform,'stage_key'=>'feeder_stage'))->get()->row();
		$commission_amount=$commission_info->commission;
		...
		foreach($info as $row)
		{
			...
			echo $lev1_num=$obj->db->select('*')->from('matrix_downline')->where(array('income_id'=>$upliner,'level'=>'1'))->get()->num_rows();
			echo $lev2_num=$obj->db->select('*')->from('matrix_downline')->where(array('income_id'=>$upliner,'level'=>'2'))->get()->num_rows();
			////////////////////
			$lev1_num_info=$obj->db->select('*')->from('matrix_downline')->where(array('income_id'=>$upliner,'level'=>'1'))->limit('1','0')->get()->row();
...
function check_upliners2_manual($info,$platform)
{
		$obj=& get_instance();
...
function check_upliners3_manual($info,$platform)
{
		$obj=& get_instance();
		$commission_info=$obj->db->select('*')->from('matrix_stage_commission_meta')->where(array('pkg_id'=>$platform,'stage_key'=>'stage_2'))->get()->row();
...
}//end check_upliners3_manual() function
function check_upliners4_manual($info,$platform)
{
		$obj=& get_instance();
		$commission_info=$obj->db->select('*')->from('matrix_stage_commission_meta')->where(array('pkg_id'=>$platform,'stage_key'=>'stage_3'))->get()->row();
...
function check_upliners5_manual($info,$platform)
{
		$obj=& get_instance();
...
function check_upliners6_manual($info,$platform)
{
		$obj=& get_instance();
...
function check_upliners7_manual($info,$platform)
{
		$obj=& get_instance();
...
function check_upliners8_manual($info,$platform)
{
		$obj=& get_instance();
		//$qr=mysql_query("select * from matrix_downline where down_id='$new_id'");
		//echo "\ncheck_upliners7";
...
function manage_matrix_downline($user_obj)
{
	$obj=& get_instance();
	$user_id=$user_obj->user_id;
	$nom_id=$user_obj->nom_id;
	$nom_leg_position=$user_obj->nom_leg_position;
